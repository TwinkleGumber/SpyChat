...
# The app should ask the user if they want to continue with the default user or create their own. (Mandatory)
...
# If a spy send a message with special words such as SOS, SAVE ME etc. (i.e. message in capital letters) you should display an appropriate message.
    ...
    for word in words:
        if word == word.upper():
            ...
            print "Recieved secret message is: " + secret_text
...
def read_chat_history():
    ...
    for chat in friends[read_for].chats:
        ...
        if chat.sent_by_me:
# print the chat history for that particular friend
...
# Make sure the age of the user is greater than 12 and less than 50 otherwise the app should display an appropriate message and exit.
...
# Print an appropriate welcome message with the name, salutation, age and rating of the spy.
        ...
        while show_menu:
# The app should display a menu with some choices.
...
# For default user the app should import the details from spy_details.py
if existing.upper() == "Y":
    start_chat(spy)
...
# It checks that the user has not entered an invalid name as input otherwise it should display a warning message and finish execution
    if len(spy.name) > 0:
# The app should ask for the salutaion the user wants to be used in front of their name.
